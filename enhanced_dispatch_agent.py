"""
Enhanced Smart Dispatch Agent with Fine-tuned Matching
Improved algorithm that considers priority, skill diversity, and workload balance.
"""
...
class Technician:
    ...
    available: bool = True
    current_priority_assignments: Dict[str, int] = None
...
class EnhancedDispatchAgent:
    """
    Enhanced dispatch agent with improved matching logic.
    """
    ...
    def load_current_assignments(self):
        """Load current assignment statistics for workload balancing"""
        ...
        try:
            query = """
                ...
                WHERE "Optimization_status" = 'completed'
                    AND "Optimized_technician_id" IS NOT NULL
                GROUP BY "Optimized_technician_id", "Priority", "Required_skill";
            """
            
            cursor.execute(query)
            for row in cursor.fetchall():
                tech_id, priority, skill, count = row
                if tech_id:
                    self.technician_assignments[tech_id]['total'] += count
                    if priority:
                        self.technician_assignments[tech_id]['by_priority'][priority] += count
                    if skill:
                        self.technician_assignments[tech_id]['by_skill'][skill] += count
    ...
    def skill_match_score(self, technician_skill: str, required_skill: str) -> float:
        """
        Enhanced skill matching with better scoring.
        Returns 0.0 to 1.0 score.
        """
        if not technician_skill or not required_skill:
            return 0.0
        
        tech_skill_lower = technician_skill.lower().strip()
        req_skill_lower = required_skill.lower().strip()
        
        # Exact match
        if tech_skill_lower == req_skill_lower:
            return 1.0
        
        # Contains match (e.g., "Network troubleshooting" contains "troubleshooting")
        if req_skill_lower in tech_skill_lower or tech_skill_lower in req_skill_lower:
            return 0.8
        
        # Keyword matching
        tech_words = set(tech_skill_lower.split())
        req_words = set(req_skill_lower.split())
        
        common_words = tech_words.intersection(req_words)
        if len(common_words) > 0:
            return 0.6 * (len(common_words) / max(len(tech_words), len(req_words)))
        
        # Related skills mapping
        related_skills = {
            'installation': ['install', 'setup', 'deploy', 'ont'],
            'repair': ['restoration', 'fix', 'maintenance', 'line'],
            'diagnosis': ['diagnostic', 'troubleshoot', 'test', 'support'],
            'network': ['connectivity', 'troubleshooting', 'support'],
            'cable': ['line', 'fiber', 'copper']
        }
    ...
    def availability_score(self, technician: Technician) -> float:
        ...
        # Penalize high utilization more
        if utilization >= 1.0:
            return 0.0
    ...
    def priority_balance_score(self, technician: Technician, dispatch: Dispatch) -> float:
        """
        Score based on how well technician's current assignments match priority distribution.
        Prevents overloading technicians with only critical dispatches.
        """
        tech_id = technician.technician_id
        assignments = self.technician_assignments[tech_id]
        
        # If technician has no assignments, give full score
        if assignments['total'] == 0:
            return 1.0
        
        # Count current priority assignments
        current_critical = assignments['by_priority'].get('Critical', 0)
        current_high = assignments['by_priority'].get('High', 0)
        current_normal = assignments['by_priority'].get('Normal', 0)
        current_low = assignments['by_priority'].get('Low', 0)
        
        # If adding this dispatch would overload on one priority, penalize
        if dispatch.priority == 'Critical':
            if current_critical > assignments['total'] * 0.7:
                return 0.5  # Too many critical already
        elif dispatch.priority == 'High':
            if current_high > assignments['total'] * 0.6:
                return 0.6
        elif dispatch.priority == 'Normal':
            if current_normal > assignments['total'] * 0.8:
                return 0.7
        
        return 1.0
    
    def skill_diversity_score(self, technician: Technician, dispatch: Dispatch) -> float:
        """
        Score based on skill diversity - prefer technicians who haven't been assigned
        too many dispatches of the same skill.
        """
        tech_id = technician.technician_id
        assignments = self.technician_assignments[tech_id]
        ...
        # Count how many times this skill has been assigned
        skill_count = assignments['by_skill'].get(dispatch.required_skill, 0)
        
        # If technician has too many of same skill, slightly penalize
        if skill_count > assignments['total'] * 0.7:
            return 0.7  # Too much of same skill
        
        return 1.0
    
    def get_available_technicians(self, required_skill: str, state: str, 
                                  appointment_date: str) -> List[Technician]:
        """Query database for available technicians"""
        cursor = self.connection.cursor()
    ...
    def check_calendar_availability(self, technician_id: str, date: str) -> bool:
        ...
        try:
            ...
            cursor.execute(query, (technician_id, date.split()[0] if date else ""))
    ...
    def score_technician(self, technician: Technician, dispatch: Dispatch) -> Dict:
        ...
        priority_balance = self.priority_balance_score(technician, dispatch)
        
        # Skill diversity (5%) - NEW
        skill_diversity = self.skill_diversity_score(technician, dispatch)
    ...
    def assign_technician(self, dispatch_id: int, technician_id: str, 
        ...
        """Update dispatch with assigned technician"""
        cursor = self.connection.cursor()
        ...
        try:
            ...
            cursor.execute("""
                SELECT "Priority", "Required_skill"
                FROM "team_core_flux"."current_dispatches"
                WHERE "Dispatch_id" = %s;
            """, (dispatch_id,))
            ...
            result = cursor.fetchone()
            if result:
                priority, skill = result
                self.technician_assignments[technician_id]['total'] += 1
                if priority:
                    self.technician_assignments[technician_id]['by_priority'][priority] += 1
                if skill:
                    self.technician_assignments[technician_id]['by_skill'][skill] += 1
    ...
    def process_dispatch(self, dispatch_id: int) -> Optional[Dict]:
        ...
        try:
            ...
            for i, candidate in enumerate(candidates, 1):
                ...
                print(f"   - Priority Balance: {candidate['priority_balance']:.2f}")
                print(f"   - Skill Diversity: {candidate['skill_diversity']:.2f}")
    ...
    def process_pending_dispatches(self, limit: int = 50, priority_first: bool = True) -> List[Dict]:
        """Process multiple pending dispatches with priority ordering"""
        cursor = self.connection.cursor()
...
def main():
    """Main function"""
    print("=" * 100)
    print("ENHANCED SMART DISPATCH AGENT")
    print("=" * 100)
    
    agent = EnhancedDispatchAgent()
    
    try:
        print("\n🚀 Processing pending dispatches (priority first)...")
        results = agent.process_pending_dispatches(limit=50, priority_first=True)
